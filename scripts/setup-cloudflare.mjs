...
export async function checkHealth(base,{fetcher=fetch,wait=ms=>new Promise(resolve=>setTimeout(resolve,ms)),attempts=6}={}){
let problem='No response';
for(let attempt=0;attempt<attempts;attempt++){
try{
const response=await fetcher(base+'/health',{signal:AbortSignal.timeout(20000),headers:{Accept:'application/json'}});
const body=await response.text();
let health;try{health=JSON.parse(body);}catch{throw Error('HTTP '+response.status+' returned a non-JSON page');}
if(!response.ok)throw Error('HTTP '+response.status);
if(health.service!=='crystal-shared-pfp')throw Error('Unexpected service response');
return health;
}catch(error){problem=error.message;if(attempt+1<attempts){console.log('Waiting for the deployed service ('+(attempt+1)+'/'+attempts+')...');await wait(3000);}}
}
throw Error('Health check at '+base+'/health failed: '+problem+'. Your Worker and database were deployed. Open that URL in your browser and share the error shown (no keys), or rerun setup after a short wait.');
}
...
let health=await checkHealth(base);
...
health=await checkHealth(base);if(health.service!=='crystal-shared-pfp'||!health.verificationConfigured)throw Error('Service health check did not pass. Rerun setup after checking the API secret.');
