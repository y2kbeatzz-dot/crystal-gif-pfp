...
async function viewerStatus(){try{const [tab]=await chrome.tabs.query({active:true,currentWindow:true});if(!tab?.id||!tab.url?.startsWith('https://www.youtube.com/'))return 'Viewer status: open YouTube to test.';const r=await chrome.tabs.sendMessage(tab.id,{type:'viewer-debug'});const d=r?.debug||{};if(d.lastError)return 'Viewer status: '+d.lastError;return 'Viewer status: channel '+(d.pageRef?'✓':'—')+' · profile '+(d.profiles>0?'✓':'—')+' · GIF '+(d.downloaded>0?'✓':'—')+' · avatar '+(d.applied>0?'✓':'—');}catch{return 'Viewer status: reload YouTube once.';}}
async function refresh(){const s=await send('state'),a=await send('account');$('preview').hidden=!s.gif;if(s.gif)$('preview').src=s.gif;else $('preview').removeAttribute('src');$('enabled').checked=s.enabled!==false;$('shared').checked=!!s.sharedEnabled;$('pick').disabled=!s.gif;$('picked').textContent=s.target?'Local picture selected.':'Choose a GIF, then select the picture you want animated on YouTube.';$('account').textContent=a.account?'Connected: '+a.account.title+(a.account.handle?' · '+a.account.handle:''):'';if(a.account&&a.account.handle&&!$('channel').value)$('channel').value=a.account.handle;$('service').textContent=a.ready?('Community viewing connected · Worker v'+a.serviceVersion+(a.publishReady?' · publishing ready':' · update to Worker v5 required')):'Shared service is not online yet. Local mode is available.';showGifCheck(parseGifData(s.gif));$('share').disabled=!a.publishReady||!currentGifInfo?.shareReady;$('delete').disabled=!a.account;$('shared').disabled=!a.ready;$('viewerstatus').textContent=await viewerStatus();}
...
action('refreshcommunity',async()=>{await send('refresh-community');const [tab]=await chrome.tabs.query({active:true,currentWindow:true});if(tab?.id)await chrome.tabs.sendMessage(tab.id,{type:'force-community-refresh'}).catch(()=>{});status('Community cache cleared. Checking this page again…');await new Promise(r=>setTimeout(r,500));});
